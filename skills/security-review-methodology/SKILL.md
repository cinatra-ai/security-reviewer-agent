...
- **Credential-shaped variables in prompt templates**: A Jinja var named `apiKey`, `token`, `secret`, `bearer`, `password` interpolated into a `system` field is a smell — even with placeholder substitution, it suggests the design expects a credential to flow through the prompt. Flag as warning.
